...
//Antes de devolver true, hay que limpiar la memoria reservada con un ClearData().
	...
	if (!u_prev) return false;
	v_prev = (float*)malloc(size * sizeof(float));
	if (!v_prev) return false;
	dens = (float*)malloc(size * sizeof(float));
	if (!dens) return false;
	dens_prev = (float*)malloc(size * sizeof(float));
	if (!dens_prev) return false;
	ClearData();
	return true;
}
...
*/
	...
	if (b == 0) {
		...
		for (unsigned j = 1; j <= N; j++) {
			//bordes x
			x[XY_TO_ARRAY(j, 0)] = x[XY_TO_ARRAY(j, 1)];
			x[XY_TO_ARRAY(j, N+1)] = x[XY_TO_ARRAY(j, N)];

			//bordes y
			x[XY_TO_ARRAY(0, j)] = x[XY_TO_ARRAY(1, j)];
			x[XY_TO_ARRAY(N+1, j)] = x[XY_TO_ARRAY(N, j)];
		}
	...
	if (b == 1) {
		...
		for (unsigned j = 1; j <= N; j++) {
			x[XY_TO_ARRAY(j, 0)] = x[XY_TO_ARRAY(j, 0)]*-1;
			x[XY_TO_ARRAY(j, N+1)] = x[XY_TO_ARRAY(j, N+1)]*-1;
		}
	...
	if (b == 2) {
		...
		for (unsigned j = 1; j <= N; j++) {
			...
			x[XY_TO_ARRAY(0, j)] = x[XY_TO_ARRAY(0, j)]*-1;
			x[XY_TO_ARRAY(N+1, j)] = x[XY_TO_ARRAY(N+1, j)]*-1;
		}
	}
	x[XY_TO_ARRAY(0, 0)] = (x[XY_TO_ARRAY(1, 0)] + x[XY_TO_ARRAY(0, 1)]) / 2;
	x[XY_TO_ARRAY(0, N + 1)] = (x[XY_TO_ARRAY(0, N)] + x[XY_TO_ARRAY(1, N+1)]) / 2;
	x[XY_TO_ARRAY(N + 1, 0)] = (x[XY_TO_ARRAY(N, 1)] + x[XY_TO_ARRAY(N + 1, 1)]) / 2;
	...
	for (int it = 0; it < 20; it++) {
		SetBounds(b, x);
			...
			FOR_EACH_CELL
				...
				x[XY_TO_ARRAY(i, j)] = ((aij)*
				 (x[XY_TO_ARRAY(i - 1, j)] + x[XY_TO_ARRAY(i, j - 1)]
						+ x[XY_TO_ARRAY(i + 1, j)] + x[XY_TO_ARRAY(i, j + 1)])+x0[XY_TO_ARRAY(i, j)]) / aii;
...
//TODO: Se aplica el campo vectorial realizando una interploación lineal entre las 4 casillas más cercanas donde caiga el nuevo valor.
	...
	FOR_EACH_CELL
	SetBounds(b, d);
	float x_0 = i - (u[XY_TO_ARRAY(i, j)] * dt*N);
	float y_0 = j - (v[XY_TO_ARRAY(i, j)] * dt*N);

	//x_0 e y_0 vienen decimales, hay que buscar las casillas
	if (x_0 < 0) {
		x_0 = 0.5;
	}
	if (x_0 > N) {
	...
	int x_0_INT=x_0, y_0_INT=y_0;
	float s = x_0 - x_0_INT;
	float t = y_0 - y_0_INT;

	//Ahora tenemos la primera casilla (la que le corresponde

	d[XY_TO_ARRAY(i, j)] = ((d0[XY_TO_ARRAY(x_0_INT, y_0_INT)] * (1-s)*(1-t)))+
							(d0[XY_TO_ARRAY(x_0_INT +1, y_0_INT)] * (s)*(1-t)) +
							(d0[XY_TO_ARRAY(x_0_INT, y_0_INT +1)] *(1-s)*(t))+
							(d0[XY_TO_ARRAY(x_0_INT +1, y_0_INT + 1)])*(s)*(t);
...
void Solver::Project(float * u, float * v, float * p, float * div)
{
	...
	//habria que poner un linsolve2 por un trigger en el teclado o algo para lo del gradiente conjugado
